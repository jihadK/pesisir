    ...
    <div class="col-md-9">
        <div class="card mb-5">
            ...
            <div class="card-body">
                ...
                @php
                    // Warehouse default: WH-LAMONGAN, fallback ke warehouse aktif pertama
                    $defaultWh = $warehouses->firstWhere('code', 'WH-LAMONGAN') ?? $warehouses->first();
                    $selectedWh = old('warehouse_id', $po->warehouse_id ?? $defaultWh?->id);
                @endphp
                {{-- Warehouse Tujuan di-hide, default WH-LAMONGAN --}}
                <input type="hidden" name="warehouse_id" value="{{ $selectedWh }}" />

                <div class="row mb-4">
                    <label class="col-form-label col-md-3 fw-semibold required">Tanggal Pembelian</label>
                    <div class="col-md-3">
                    ...
                    <label class="col-form-label col-md-3 fw-semibold">Rencana Belanja</label>
                    <div class="col-md-3">
        ...
        <div class="card mb-5">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-row-bordered align-middle gy-3" id="tbl_items" style="min-width:960px">
                        <thead>
                            <tr class="fw-bold text-muted bg-light">
                                ...
                                <th class="text-end" style="min-width:130px">Diskon</th>
                        ...
                        <tfoot class="fw-bold">
                            <tr><td colspan="4" class="text-end fs-4">TOTAL</td><td class="text-end fs-4 text-primary" id="ft_total">Rp 0</td><td></td></tr>
                        </tfoot>
                    </table>
                </div>
...
function addItemRow(prefill = null) {
    ...
    tr.innerHTML = `
        ...
        <td><div class="input-group input-group-sm"><span class="input-group-text">Rp</span><input type="text" name="items[${idx}][discount_amount]" class="form-control form-control-sm form-control-solid text-end discount" placeholder="0" /></div></td>
    ...
    maskRupiah($tr.find('.discount')[0]);
    $tr.find('.qty-gram, .price-kg, .discount').on('input', () => recalcRow(tr));
    ...
    if (prefill) {
        ...
        $tr.find('.discount').val(Math.round(prefill.discount_amount || 0).toLocaleString('id-ID'));
        recalcRow(tr);
    }
}
...
function recalcRow(tr) {
    ...
    const disc = unmask(tr.querySelector('.discount').value);
    const sub = Math.max(0, qty * price / 1000 - disc);
